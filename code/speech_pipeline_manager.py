...
# Append the absolute path to `mcp_server` (2 levels up then into mcp_server)
...
# def call_mcp(prompt):
#     """
#     Handles both normal prompts and function-call prompts like:
#     CALL place_order(item="pizza", quantity=2)
#     """
#     try:
#         # Step 1: Send prompt to MCP server
#         res = requests.post("http://172.17.0.1:9000/mcp", json={"query": prompt})
...
#         # Step 3: Return parsed response or fallback message
...
# print("🧠 LLM returned:", llm_output)

# # Now send to MCP
# response = call_mcp(llm_output)
...
class RunningGeneration:
    ...
    def handle_word(self, word: str):
        """Called on each audio chunk (word)."""
        # Optional: Log or do something per chunk
        pass

    def handle_start(self):
        """Called when TTS starts speaking."""
        # Optional: set flags, logging, etc.
        pass

    def handle_end(self):
        """Called when TTS finishes speaking."""
        self.completed = True
        self.tts_quick_finished_event.set()
...
class SpeechPipelineManager:
    ...
    def __init__(
        ...
        self.menu = MenuManager(path="data/enhanced_menu.json")
        ...
        # Handle case where LLM inference time is None (failed initialization)
        llm_time = self.llm_inference_time if self.llm_inference_time is not None else 0.0
        self.full_output_pipeline_latency = llm_time + self.audio.tts_inference_time
        logger.info(f"🗣️⏱️ Full output pipeline latency: {self.full_output_pipeline_latency:.2f}ms (LLM: {llm_time:.2f}ms, TTS: {self.audio.tts_inference_time:.2f}ms)")
    ...
    def process_prepare_generation(self, txt: str):
        ...
        logger.info(f"🧪 [Gen {new_gen_id}] Preparing new generation for: '{txt[:50]}...'")
        ...
        try:
            logger.info(f"🧪 [Gen {new_gen_id}] Calling MCP...")
            
            # 🔗 MCP Call
            response = call_mcp(txt)
            logger.info(f"🧪 [Gen {new_gen_id}] MCP response: {response}")

            def one_shot_generator():
                yield response

            self.running_generation.llm_generator = one_shot_generator()

            self.generator_ready_event.set()

        except Exception as e:
            logger.exception(f"🧪 [Gen {new_gen_id}] Failed in process_prepare_generation: {e}")
            self.running_generation = None

# --- Processing Methods ---
    # def process_prepare_generation(self, txt: str):
    #     logger.info("🔥 ENTERED process_prepare_generation")
    #     id_in_spec = self.generation_counter + 1
    #     self.check_abort(txt, wait_for_finish=True, abort_reason=f"process_prepare_generation for new id {id_in_spec}")
    ...
    #     logger.info(f"🧠 [Gen {new_gen_id}] Sending prompt to prepare_generation...")

    #     # ✅ Launch async LLM → MCP → TTS logic
    #     asyncio.create_task(self.prepare_generation(txt))
    ...
    # async def prepare_generation(self, prompt: str):
    #     logger.info("🎯 ENTERED prepare_generation")

    #     logger.info("🎙️ Received prompt: %s", prompt)
    ...
    #         logger.warning("🚫 Generation already running")
        ...
        # self.running_generation = RunningGeneration(prompt=prompt)

        # try:
        #     llm_output = call_llm(prompt)
        #     logger.info("🧠 LLM Output: %s", llm_output)

        #     if not llm_output or not llm_output.startswith("CALL"):
        #         self.running_generation.quick_answer = f"🤖 Echo: {prompt}"
        #     else:
        #         self.running_generation.quick_answer = call_mcp(llm_output)

        #     logger.info("📤 Final Response: %s", self.running_generation.quick_answer)

        #     self.running_generation.stream = self.tts.start(
        #         self.running_generation.quick_answer,
        #         on_word=self.running_generation.handle_word,
        #         on_start=self.running_generation.handle_start,
        #         on_end=self.running_generation.handle_end,
        #     )
